...
SQL_QUERY="UPDATE ${TABLE_NAME} SET is_logged_in = TRUE, login_time = TRUNC(EXTRACT(EPOCH FROM NOW())) WHERE client_id = '${CLIENT_ID}';"
...
if [ $? -eq 0 ]; then
  echo "Successfully updated is_logged_in and login_time for client_id: ${CLIENT_ID}"
else
